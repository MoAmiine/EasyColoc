                ...
                @else
                    <div class="space-y-3">
                        @foreach ($colocation->depenses as $depense)
                            <div
                                class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl hover:bg-indigo-50/50 transition-colors group">
                                <div class="flex items-center gap-4">
                                    <div
                                        class="w-12 h-12 rounded-xl bg-white text-indigo-600 flex items-center justify-center font-bold shadow-sm">
                                ...
                                <div class="flex items-center gap-4">
                                    <div class="text-right">
                                        <p class="font-black text-slate-900 text-lg">
                                            {{ number_format($depense->amount, 2, ',', ' ') }} €</p>
                                        @if ($depense->user_id === Auth::id())
                                            <span class="text-[10px] font-bold text-emerald-600 uppercase">Payé par
                                                vous</span>
                                        @endif
                                    </div>
                                    ...
                                    @if ($depense->user_id === Auth::id() || Auth::id() === $colocation->owner_id)
                                        <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <form action="{{ route('depenses.destroy', [$colocation, $depense]) }}"
                                                method="POST" onsubmit="return confirm('Supprimer cette dépense ?');"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"

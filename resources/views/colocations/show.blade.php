<x-app-layout>
    ...
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                ...
                <p class="text-4xl font-black text-slate-900">{{ number_format($colocation->depenses->sum('amount'), 2, ',', ' ') }} €</p>
            </div>
            <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div id="content-members" class="p-8">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-xl font-black text-slate-900 italic uppercase tracking-tight">Membres de la colocation</h3>
                    @if(Auth::id() === $colocation->owner_id)
                        <span class="px-3 py-1 bg-indigo-100 text-indigo-600 text-[10px] font-black uppercase rounded-lg">Vous êtes Owner</span>
                    @endif
                </div>
                    ...
                    @foreach($colocation->users as $user)
                        <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-xl bg-indigo-100 text-indigo-600 flex items-center justify-center font-black text-lg">
                            ...
                            @if(Auth::id() === $colocation->owner_id && $user->id !== Auth::id())
                                <form action="
                                    @csrf @method('DELETE')
                                    <button type="submit" class="text-rose-500 hover:bg-rose-50 p-2 rounded-xl transition-colors" title="Retirer">
                ...
                @if(Auth::id() === $colocation->owner_id)
                    <div class="mt-8 pt-8 border-t border-slate-100">
                        <h4 class="text-sm font-black text-slate-900 uppercase tracking-widest mb-4">Inviter un nouveau membre</h4>
                        <form action="
                            @csrf
                            <input type="email" name="email" placeholder="email@exemple.com" 
                                ...
                                required>
                            <button type="submit" class="px-6 py-3 bg-indigo-600 text-white font-black uppercase tracking-widest rounded-2xl hover:bg-indigo-700 transition text-sm">
                                Inviter
                            </button>
                        </form>
                    </div>
            ...
            <div id="content-expenses" class="hidden p-8">
                ...
                @if($colocation->depenses->count() === 0)
                    <div class="text-center py-12">
                        ...
                        <a href="{{ route('depenses.create', $colocation) }}" class="text-indigo-600 font-bold text-sm mt-2 inline-block hover:underline">
                ...
                @else
                    <div class="space-y-3">
                        @foreach($colocation->depenses as $depense)
                            <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl hover:bg-indigo-50/50 transition-colors group">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                            Par {{ $depense->user->firstname ?? 'Inconnu' }} • {{ $depense->spent_at->format('d/m/Y') }}
                                ...
                                <div class="flex items-center gap-4">
                                    <div class="text-right">
                                        <p class="font-black text-slate-900 text-lg">{{ number_format($depense->amount, 2, ',', ' ') }} €</p>
                                    ...
                                    @if($depense->user_id === Auth::id() || Auth::id() === $colocation->owner_id)
                                        <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <form action="{{ route('depenses.destroy', [$colocation, $depense]) }}" method="POST" 
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-2 text-rose-500 hover:bg-rose-100 rounded-lg transition-colors" title="Supprimer">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                                    </svg>
                                                </button>
        ...
        @if(Auth::id() === $colocation->owner_id)
            <div class="flex items-center justify-end gap-4">
                ...
                <form action="{{ route('colocation.delete', $colocation) }}" method="POST" onsubmit="return confirm('Supprimer définitivement cette colocation ?');">
                    @csrf @method('DELETE')
    ...
    <script>
        function switchTab(tabName) {
            ...
            document.getElementById('tab-members').classList.remove('text-indigo-600', 'border-b-2', 'border-indigo-600', 'bg-indigo-50/50');
            document.getElementById('tab-members').classList.add('text-slate-400');
            document.getElementById('tab-expenses').classList.remove('text-indigo-600', 'border-b-2', 'border-indigo-600', 'bg-indigo-50/50');
